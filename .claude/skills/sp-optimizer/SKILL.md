...
**Eligibility & workload-quality gate (these override `target_met` and block `apply`).**
A score is only trusted when a *valid, representative* workload backs it. The run
stops with a truthful non-plan status — and the agent is **not** asked to change
the procedure — when:
- a **discovery-time precondition** blocks the run: a table-valued or JSON/XML
  parameter that can't be a scalar literal (`requires_curated_workload`), a
  secret-bearing parameter (`requires_sensitive_input`), a paired setup/teardown
  proc (`requires_setup`), a missing server feature such as Full-Text Search
  (`blocked_prerequisite`), or an unbounded bulk generator not explicitly opted
  into (`requires_curated_workload`);
- **no capture was analyzable** — every call failed or produced no plan
  (`capture_failed` / `not_analyzable`), e.g. a control/error-handling proc;
- the **actual workload was all-empty** — every executed call returned zero rows
  (`empty_workload`), so the measured plans are not representative.

Only `scorable` (`analyzed`) plans feed the aggregate, the good fraction, and any
decision to `apply`. `evaluate` reports `terminal_status`, `eligible_for_apply`,
and `representative_workload` so you can see exactly why a run stopped.
...
5. **Never mine or persist secrets.** Password/token/key parameters are detected by name; they are never read from the database, their values are redacted from labels/session/report, and a valid call requires a caller-supplied secret (`requires_sensitive_input`).
6. **Bound the cost of actual capture.** Every actual call runs under a per-combo command timeout (`--command-timeout`, default 120s) that cancels the server request; recognized unbounded bulk generators / storage rebuilds require an explicit `SP_OPT_ALLOW_BULK=1` opt-in before they run.

## Workload eligibility & preconditions

The workload is only as trustworthy as it is *valid* and *representative*.
`scripts/eligibility.py` is the single, procedure-agnostic classifier (no
per-proc constants) that keeps a non-plan failure from being scored as a bad
query plan:

- **Type-aware values (Issue 5).** Synthesized numerics respect the declared SQL
  range/precision/scale (`tinyint` 0..255, `decimal(p,s)`, …). A value that can't
  fit its type is an `invalid_input` discovery defect, never a plan score.
- **Cross-parameter validity (Issue 6).** Role/user parameters that collide or a
  special/fixed principal (`db_*`, `sys`, `public`) → `requires_curated_workload`.
- **TVPs & structured payloads (Issues 11, 12).** Table-valued params and
  JSON/XML scalars that can't be a literal → `requires_curated_workload`; supply
  a fixture with `SP_OPT_COMBOS`.
- **Secrets (Issue 10).** See safety rule 5.
- **Server prerequisites (Issue 4).** A Full-Text-dependent proc on a server
  without the component → `blocked_prerequisite`; the first shared failure
  short-circuits the remaining combos.
- **Lifecycle (Issue 9).** Paired setup/teardown procs → `requires_setup`;
  declare a contract with `SP_OPT_SETUP_SQL` / `SP_OPT_TEARDOWN_SQL`.
- **Representativeness (Issue 2).** An all-empty actual workload can't be
  `target_met`; opt in with `SP_OPT_ALLOW_EMPTY=1` for a deliberately empty test.

Environment opt-ins: `SP_OPT_COMBOS`, `SP_OPT_ALLOW_BULK`, `SP_OPT_ALLOW_EMPTY`,
